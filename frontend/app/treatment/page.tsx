                    ...
                    <div className="max-w-5xl mx-auto">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 mb-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                ...
                                <div>
                                    <label className="block text-gray-300 mb-3 font-medium text-sm">Patient Age</label>
                                    <div className="space-y-3">
                                        ...
                                        <input type="range" min="1" max="100" value={patientAge} onChange={(e) => setPatientAge(Number(e.target.value))}
                                            ...
                                            style={{ background: `linear-gradient(to right, #C5757C 0%, #C5757C ${patientAge}%, rgba(255,255,255,0.1) ${patientAge}%, rgba(255,255,255,0.1) 100%)` }}
                                        />
                                    </div>
                                </div>

                                <div>
                                    <label className="block text-gray-300 mb-3 font-medium text-sm">AI Confidence</label>
                                    <div className="space-y-3">
                                        <div className="flex items-baseline justify-between">
                                        ...
                                        <input type="range" min="0.5" max="1" step="0.01" value={confidence} onChange={(e) => setConfidence(Number(e.target.value))}
                                            className="w-full h-2 bg-white/10 rounded-full appearance-none cursor-pointer"
                                            style={{ background: `linear-gradient(to right, #F9AAAD 0%, #F9AAAD ${(confidence - 0.5) * 200}%, rgba(255,255,255,0.1) ${(confidence - 0.5) * 200}%, rgba(255,255,255,0.1) 100%)` }}
                                        />
                                    </div>
                                </div>
                            </div>
                            ...
                            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleSimulation} disabled={loading}
                                className="w-full mt-8 px-8 py-4 bg-gradient-to-r from-[#C5757C] to-[#F9AAAD] text-white font-bold rounded-xl hover:shadow-2xl hover:shadow-[#C5757C]/50 transition-all disabled:opacity-50 flex items-center justify-center gap-3">
                        ...
                        {error && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-red-500/20 border border-red-500/50 rounded-xl p-6 mb-8">
                            ...
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                                ...
                                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 mb-8">
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        ...
                                        <div className="text-center p-4 bg-white/5 rounded-lg">
                                            <p className="text-cyan-300 text-sm mb-1">Severity</p>
                                    ...
                                    <div className="mt-4 p-4 bg-orange-500/20 border border-orange-500/50 rounded-lg">
                                        <p className="text-orange-200 text-sm"><strong>⏰ Urgency:</strong> {results.urgency}</p>
                                    </div>
                                </div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-6">
                                        {results.recommendations.map((treatment, index) => (
                                            <button key={index} onClick={() => setSelectedTreatment(index)}
                                                className={`px-4 py-2 rounded-lg font-medium transition-all ${selectedTreatment === index ? 'bg-gradient-to-r from-[#C5757C] to-cyan-500 text-white shadow-lg' : 'bg-white/5 text-gray-300 hover:bg-white/10'}`}>
                                    ...
                                    {results.recommendations[selectedTreatment] && (
                                        <motion.div key={selectedTreatment} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                            <div className="border-b border-white/20 pb-4">
                                                ...
                                                <p className="text-gray-300">{results.recommendations[selectedTreatment].description}</p>
                                                <div className="flex flex-wrap gap-2 mt-3">
                                                    <span className="px-3 py-1 bg-emerald-500/20 border border-emerald-500/50 rounded-full text-emerald-200 text-sm">{results.recommendations[selectedTreatment].type}</span>
                                                    <span className="px-3 py-1 bg-cyan-500/20 border border-cyan-500/50 rounded-full text-cyan-200 text-sm">{results.recommendations[selectedTreatment].success_rate}% Success Rate</span>
                                                </div>
                                            </div>

                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                <div className="bg-gradient-to-br from-[#C5757C]/10 to-transparent rounded-xl p-4 border border-emerald-500/20">
                                                ...
                                                <div className="bg-gradient-to-br from-cyan-500/10 to-transparent rounded-xl p-4 border border-cyan-500/20">
                                                    <p className="text-cyan-300 text-sm mb-1">🏥 Recovery</p>
                                                ...
                                                {Object.entries(results.recommendations[selectedTreatment].survival_rates).map(([key, value]) => (
                                                    <div key={key} className="mb-3">
                                                        <div className="flex justify-between text-sm mb-1">
                                                            <span className="text-gray-300">{key.replace('_', '-Year ')}</span>
                                                ...
                                                <div className="bg-white/5 rounded-xl p-6 border border-red-500/20">
                                                    ...
                                                    <ul className="space-y-2">
                                                        {results.recommendations[selectedTreatment].side_effects.map((e, i) => (
                                                            <li key={i} className="flex items-start text-gray-300"><span className="text-red-400 mr-2">•</span>{e}</li>
                                                        ))}
                                                    </ul>
                                                </div>
                                                <div className="bg-white/5 rounded-xl p-6 border border-emerald-500/20">
                                                    ...
                                                    <ul className="space-y-2">
                                                        {results.recommendations[selectedTreatment].recommended_for.map((r, i) => (
                                                            <li key={i} className="flex items-start text-gray-300"><span className="text-[#F9AAAD] mr-2">✓</span>{r}</li>
                                                        ))}
                                                    </ul>
                                                </div>
                                            </div>
                                ...
                                <div className="mt-6 flex justify-end">
                                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                                        ...
                                        className="px-6 py-3 bg-gradient-to-r from-[#C5757C] to-cyan-500 text-white font-semibold rounded-lg">
                                        ⬇️ Download Treatment PDF
                                    </motion.button>
                                </div>

                                <div className="mt-6 bg-gradient-to-r from-[#C5757C]/10 to-cyan-500/10 border border-emerald-500/30 rounded-xl p-5">
                                    <p className="text-white text-sm"><strong className="text-[#F9AAAD]">⚕️ Disclaimer:</strong> {results.disclaimer}</p>
                                </div>
                            </motion.div>
                        )}
                    </div>
