                ...
                <div className="container mx-auto px-6 pt-20 pb-12 relative z-10">
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
                        <h1 className="text-5xl font-bold mb-4">
                            <span className="bg-gradient-to-r from-[#528DCB] to-[#4B78A0] text-transparent bg-clip-text">{t('about_title')}</span>
                        </h1>
                        <p className="text-[#6A7F92] text-lg max-w-2xl mx-auto">{t('about_mission_text')}</p>
                    </motion.div>
                    ...
                    <div className="max-w-5xl mx-auto space-y-12">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            ...
                            <p className="text-[#6A7F92] leading-relaxed text-lg">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            ...
                            <p className="text-[#6A7F92] mb-6">Brain tumors account for 85–90% of all primary CNS tumors. Early detection is critical, but:</p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {[
                                    ...
                                    <motion.div key={i} whileHover={{ y: -5 }} className="bg-white/80 rounded-xl p-5 border border-[#528DCB]/20">
                                        <div className="text-3xl mb-2">{item.icon}</div>
                                        <h3 className="text-[#1a1a2e] font-bold text-lg mb-1">{item.title}</h3>
                                        <p className="text-[#6A7F92] text-sm">{item.desc}</p>
                                    </motion.div>
                                ))}
                            </div>
                        </motion.div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                {[
                                    ...
                                    <motion.div key={i} whileHover={{ scale: 1.05 }} className={`bg-gradient-to-br ${item.color} to-transparent rounded-xl p-5 border text-center`}>
                                        ...
                                        <p className="text-[#6A7F92] text-sm mt-1">{item.desc}</p>
                                    </motion.div>
                                ))}
                            </div>
                        </motion.div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {[
                                    ...
                                    <div key={i} className="bg-white/80 rounded-xl p-4 border border-[#528DCB]/20 text-center">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            ...
                            <p className="text-[#6A7F92] mb-6">The entire interface and generated reports are available in three languages:</p>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {[
                                    ...
                                    <div key={i} className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 text-center">
                                        ...
                                        <p className="text-[#528DCB] text-sm mt-1">{item.status}</p>
                                    </div>
                                ))}
                            </div>
                        </motion.div>
                        ...
                        {/* Future Vision */}
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            className="bg-gradient-to-r from-[#528DCB]/10 to-cyan-100/30 backdrop-blur-xl border border-emerald-200 rounded-2xl p-8">
                            <h2 className="text-3xl font-bold text-[#1a1a2e] mb-4">🔬 Future Vision</h2>
                            <p className="text-[#6A7F92] mb-4">Expanding beyond brain tumors to detect tumors across the entire body:</p>
                            <div className="flex flex-wrap gap-3">
                                {['🫁 Lung Tumors', '🩻 Bone Tumors', '🔬 Liver & Abdominal', '📡 Pan-India Deployment', '🏛️ Government Integration'].map((item, i) => (
                                    <span key={i} className="px-4 py-2 bg-white/70 rounded-full text-[#1a1a2e] text-sm border border-[#528DCB]/20">{item}</span>
                                ))}
                            </div>
                        </motion.div>
                    </div>
