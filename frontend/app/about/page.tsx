            ...
            <div className="min-h-screen relative overflow-hidden">
                ...
                <div className="container mx-auto px-6 pt-20 pb-12 relative z-10">
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
                        ...
                        <p className="text-[#683A46]/60 text-lg max-w-2xl mx-auto">{t('about_mission_text')}</p>
                    </motion.div>
                    ...
                    <div className="max-w-5xl mx-auto space-y-12">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            ...
                            <h2 className="text-3xl font-bold text-[#462037] mb-4">🎯 {t('about_mission')}</h2>
                            <p className="text-[#683A46]/70 leading-relaxed text-lg">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            className="bg-white/80 backdrop-blur-sm border border-[#C5757C]/15 shadow-sm rounded-2xl p-8">
                            <h2 className="text-3xl font-bold text-[#462037] mb-6">📌 The Problem</h2>
                            <p className="text-[#683A46]/70 mb-6">Brain tumors account for 85–90% of all primary CNS tumors. Early detection is critical, but:</p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {[
                                    ...
                                    <motion.div key={i} whileHover={{ y: -5 }} className="bg-white/70 rounded-xl p-5 border border-[#C5757C]/15">
                                        <div className="text-3xl mb-2">{item.icon}</div>
                                        <h3 className="text-[#462037] font-bold text-lg mb-1">{item.title}</h3>
                                        <p className="text-[#683A46]/60 text-sm">{item.desc}</p>
                                    </motion.div>
                                ))}
                            </div>
                        </motion.div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            className="bg-white/80 backdrop-blur-sm border border-[#C5757C]/15 shadow-sm rounded-2xl p-8">
                            <h2 className="text-3xl font-bold text-[#462037] mb-6">🧠 Detection Capabilities</h2>
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                {[
                                    ...
                                    <motion.div key={i} whileHover={{ scale: 1.05 }} className={`bg-gradient-to-br ${item.color} to-transparent rounded-xl p-5 border text-center`}>
                                        <p className="text-[#462037] font-bold text-lg">{item.type}</p>
                                        <p className="text-[#683A46]/60 text-sm mt-1">{item.desc}</p>
                                    </motion.div>
                                ))}
                            </div>
                        </motion.div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            className="bg-white/80 backdrop-blur-sm border border-[#C5757C]/15 shadow-sm rounded-2xl p-8">
                            <h2 className="text-3xl font-bold text-[#462037] mb-6">⚙️ Technology</h2>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {[
                                    ...
                                    <div key={i} className="bg-white/70 rounded-xl p-4 border border-[#C5757C]/15 text-center">
                                        <p className="text-[#462037] font-semibold text-sm">{item.tech}</p>
                                        <p className="text-[#683A46]/50 text-xs mt-1">{item.role}</p>
                                    </div>
                                ))}
                            </div>
                        </motion.div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            className="bg-white/80 backdrop-blur-sm border border-[#C5757C]/15 shadow-sm rounded-2xl p-8">
                            <h2 className="text-3xl font-bold text-[#462037] mb-6">🌐 Multilingual Support</h2>
                            <p className="text-[#683A46]/70 mb-6">The entire interface and generated reports are available in three languages:</p>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {[
                                    ...
                                    <div key={i} className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-5 text-center">
                                        ...
                                        <p className="text-[#462037] font-bold">{item.lang}</p>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                            ...
                            <h2 className="text-3xl font-bold text-[#462037] mb-4">🔬 Future Vision</h2>
                            <p className="text-[#683A46]/70 mb-4">Expanding beyond brain tumors to detect tumors across the entire body:</p>
                            <div className="flex flex-wrap gap-3">
                                {['🫁 Lung Tumors', '🩻 Bone Tumors', '🔬 Liver & Abdominal', '📡 Pan-India Deployment', '🏛️ Government Integration'].map((item, i) => (
                                    <span key={i} className="px-4 py-2 bg-white/10 rounded-full text-white text-sm border border-[#C5757C]/20">{item}</span>
                                ))}
                            </div>
                        </motion.div>
                    </div>
